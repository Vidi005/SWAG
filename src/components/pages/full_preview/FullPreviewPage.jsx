...
const FullPreviewPage = ({ t }) => {
  ...
  if (getSavedWebPreview()?.includes('<html')) {
    return (
      <iframe title="Web Preview" srcDoc={getSavedWebPreview()} className="h-screen w-full overflow-auto duration-200" frameborder="0"></iframe>
    )
    ...
    return (
      <article className="grid grid-flow-row items-center justify-center gap-4 md:gap-8 lg:gap-16 w-full h-screen p-4 md:p-8 lg:p-16 bg-cyan-50 dark:bg-gray-900 duration-200 overflow-y-auto">
        <h1 className="text-center text-cyan-900 dark:text-white">Web Preview</h1>
        <h3 className="flex flex-col justify-center text-center">
          <span className="text-cyan-700 dark:text-gray-200 p-1">No preview available.</span>
          <Link to="/"><span className="text-blue-700 dark:text-gray-300 hover:text-blue-400 dark:hover:text-gray-500 active:text-purple-700 dark:active:text-purple-500 p-1 underline">{t('<- Go Home')}</span></Link>
        </h3>
      </article>
    )
  }
