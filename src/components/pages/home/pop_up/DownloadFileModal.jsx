...
const DownloadFileModal = ({ t, inputRef, isDialogOpened, downloadAsHTML, downloadAsZip, cancelDownload }) => (
  <Transition appear show={isDialogOpened} as={Fragment}>
    <Dialog initialFocus={inputRef} className={"download-file-dialog relative z-10"} onClose={cancelDownload}>
      ...
      >
        <Dialog.Panel className={"flex flex-col items-center min-h-full max-w-5xl mx-4 bg-cyan-500 dark:bg-gray-700 text-cyan-900 dark:text-white shadow-lg dark:shadow-white/50 rounded-lg overflow-hidden"}>
          ...
          <section className="flex flex-col items-center w-full p-4 bg-cyan-50 dark:bg-gray-900 text-base lg:text-lg">
            <p className="text-justify text-cyan-900 dark:text-white leading-tight">{t('download_file_confirmation_desc')}</p>
            <br />
