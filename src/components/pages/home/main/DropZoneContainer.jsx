  ...
  if (genAIInput === 'multimodal') {
            ...
            {currentImgURLs.length === 0
              ? (
                <div className="dropzone grow w-full border-2 border-dashed border-cyan-700 dark:border-gray-300 grid items-center justify-center p-2 rounded-lg duration-200" {...getRootProps()}>
                  <h3 className="font-normal text-lg text-center text-cyan-900 dark:text-gray-100 duration-200">{t('drop_image')}</h3>
                </div>
              )
            : (
                <div className="image-preview-container flex-auto h-0 max-w-full flex flex-nowrap items-center gap-2 overflow-x-auto">
                  {currentImgURLs.map((currentImgURL, index) => (
                    <div key={index} className="image-preview-item relative h-full p-1 duration-200">
                      <img src={currentImgURL} alt="Image Preview" className="h-full object-contain duration-200 rounded-md shadow-md dark:shadow-white/50 overflow-hidden" />
                      <span className="absolute grid items-center justify-center text-center font-mono bg-gray-500/75 hover:bg-gray-200 dark:hover:bg-gray-700 text-xs text-white top-0 right-0 aspect-square px-1.5 cursor-pointer rounded-full duration-200" onClick={() => removeCurrentImage(index)}>X</span>
                    </div>
                  ))}
    ...
    return (
      <div className="input-mode-info grow w-full bg-orange-200 dark:bg-orange-800 grid items-center justify-center p-2 rounded-lg duration-200">
        <h5 className="font-normal text-xs md:text-sm text-center text-orange-900 dark:text-orange-50 duration-200">
